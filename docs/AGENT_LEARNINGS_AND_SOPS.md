...
| **Education desk (planned)** | Tamil Nadu Plus Two group selection + official textbook PDF finders. Plan: `.cursor/plans/tn-plus-two-education-desk.plan.md`. |
...
- **Education / Plus Two:** do not clone print booklets as one article; cluster as pillars + one-intent pages. Never rehost SCERT textbooks. Marketing landings (`/class-11-groups-chennai`, `/plus-two-textbooks-pdf`) must not reuse pillar titles.
...
2. **Privacy / Cookies must use AdSense-required wording:** third parties (including Google) may place/read cookies, web beacons, or IPs because of ad serving; link [How Google uses data when you use our partners’ sites or apps](https://policies.google.com/technologies/partner-sites); link [Ads Settings](https://adssettings.google.com) and [aboutads.info](https://www.aboutads.info); do not pass PII to Google. Keep **COPPA under-13** language even if the site also says it is not for under-18 profiling.
3. **`ads.txt`:** publisher line `google.com, pub-…, DIRECT, f08c47fec0942fa0` (repo default `pub-5760699639501978`). Do not invent a different ID.
4. AdSense script: client id is resolved with a repo default so `adsbygoogle.js` can load for site verification; **display units** stay env-gated (`NEXT_PUBLIC_ADSENSE_SLOT_*`). No EU CMP if product decision is **non-EU focus**—still use honest cookie/third-party copy globally.
5. Before re-requesting review: keep homepage **auto modals off** (`NEXT_PUBLIC_SITE_MODAL_AUTO` / newsletter auto unset) unless Google has already approved the site; do not put Google ads on thin/legal/auth screens.
...
| **TN Plus Two education desk** | `.cursor/plans/tn-plus-two-education-desk.plan.md`, `.cursor/rules/tn-plus-two-education.mdc` |

---

## 6. Changelog

- **2026-09-04:** AdSense content/UX **P0–P2 implemented** (deploy this commit). Custom area packs: Adyar, T. Nagar, Ambattur (+ OMR). IAS shelf expanded (EN+TA); 4 news slugs HEAD 200; gold-history 3 paras; salary `GuideDisclaimer kind="money"`; 13 `chennaiRelevance` fills; glossary +8 terms; WhatsApp guide vs join; quiz FAQ; EN Plus Two no mixed Tamil; topic empty-state without ads. Cabinet sitemap = 8 hubs + IAS (10); minister pages stay `noindex`. **Rule:** do not invent Area Sabha meetings, IAS org-charts, or salary JSON-LD; keep auto-modals and article AdSense slots off until review.
- **2026-09-04:** AdSense content/UX **P0 executed in repo** — footer/nav accuracy, partner-ad deny-list (`/search`, WhatsApp partners, civic preview), civic sitemap = hub + `status === "live"` only, Area Sabha/zone dashboard/zonal office `noindex`, cabinet sitemap gated, minister `noindex`. Plan: `.cursor/plans/adsense-content-ux-gaps.plan.md`. **Rule:** do not invent Area Sabha meetings, minister facts, or salary JSON-LD; keep auto-modals and article AdSense slots off.
- **2026-09-04:** AdSense content/UX **implementation plan** — `.cursor/plans/adsense-content-ux-gaps.plan.md` (P0 footer/nav, ads deny-list, Area Sabha `noindex`, cabinet/Plus Two sitemap+robots gates; P1 area-hub uniqueness + IAS/gold/salary copy). Legal/`ads.txt` plan remains completed. Audit: `docs/ADSENSE_CONTENT_UX_AUDIT.md`. **Rule:** next production deploy must not dump 70 thin minister URLs into the live sitemap without `noindex` or expanded copy.
- **2026-09-04:** AdSense **content and user experience** page-wise audit saved as `docs/ADSENSE_CONTENT_UX_AUDIT.md` (pointer from `ADSENSE_PRE_FLIGHT.md`). Live site: unique news/guides OK; risks are empty civic shells (`/civic-tools/area-sabha`), templated area hubs, misleading footer tags, and TN Plus Two + Council of Ministers **404 on production** (70 thin minister URLs in repo). **Rule:** AdSense reviewers see the live site — do not ship thin template farms or empty trackers into sitemap; keep ads off legal/search/forms/empty tools; keep auto-modals off until approved.
- **2026-08-29:** News — Hindu-style inline **Also Read** mid-report (`ArticleAlsoRead` + `ArticleProse` `inlineAlsoRead`). Uses first category-related article after the lead (or after paragraph 2 when the lead is long); Tamil label `இதையும் படிக்க:`. End cluster / sidebar related rails unchanged. **Rule:** keep mid-article Also Read inside civic-editorial prose — do not invent a parallel related-UI chrome.
- **2026-08-29:** Jobs — **South Indian Restaurant (Navalur)** flyer split into **five separate** open roles (Manager, Chef, Shift In-Charge, Kitchen Assistant, Team Member) via `db:seed:chennai-job:south-indian-restaurant-navalur(:live)`. Flyer `public/images/listings/south-indian-restaurant-hiring-navalur-2026.png`. Contacts 81245 37432 / 87549 73833. Live: `/chennai-jobs/restaurant-manager-south-indian-restaurant-navalur-chennai`, `/chennai-jobs/south-indian-chef-navalur-chennai`, `/chennai-jobs/shift-in-charge-south-indian-restaurant-navalur-chennai`, `/chennai-jobs/kitchen-assistant-south-indian-restaurant-navalur-chennai`, `/chennai-jobs/team-member-south-indian-restaurant-navalur-chennai`. **Rule:** multi-role F&B flyers with distinct salaries get **one JobPosting slug per role** (keyword title + Navalur + monthly salary fields) so Google JobPosting / hub SEO can index each vacancy; do not bundle them into a single combined listing.
- **2026-08-25:** TN Council of Ministers desk **live in repo** — `/guides/tn-council-of-ministers` + 35 minister pages + departments + Chennai shelf + IAS shelf + official-sources + Tamil twin (`/guides/tn-council-of-ministers-tamil/*`). PR 40 PDF self-hosted. Sitemap + `llms.txt`. Seed `db:seed:tn-cabinet-expansion-may-2026(:live)` → `/chennai-local-news/tamil-nadu-cabinet-expansion-35-ministers-may-2026`. **Rule:** roster in `ministers-may-2026.ts`; civic disclaimer; no fake minister JSON-LD.
- **2026-08-24:** TN Council of Ministers desk **planned** (not built) — 35-minister roster from Lok Bhavan PR 40 (21 May 2026); hub `/guides/tn-council-of-ministers` + minister/department/Chennai lookup pages. Canonical plan: `.cursor/plans/tn-council-of-ministers.plan.md`. Existing news article covers PR 38 (16 May, ~12 ministers) only — do not overwrite; add expansion seed or hub cross-link. **Rule:** state government reference desks live under `/guides/tn-*`; roster data in typed content files; verify against self-hosted Lok Bhavan PDF before acting.
- **2026-08-24:** Classifieds — **2015 Hero Pleasure** (single owner, lady-driven, original RC; K.K. Nagar; 88389 22250) via `db:seed:hero-pleasure-2015-kk-nagar(:live)` → `/chennai-classifieds/hero-pleasure-2015-single-owner-kk-nagar`. Photos `public/images/classifieds/hero-pleasure-2015-kk-nagar-{front,side}.png`. New **Vehicles** hub filter; `featured` + `image_urls` on `classified_listings`; homepage **Featured listings** rail. **Rule:** vehicle classifieds do not invent asking price or km; self-host seller photos; pin with `featured` rather than a hardcoded slug.
- **2026-08-23:** Events — **Made of Chennai Food and Music Festival** (The Hindu Made of Chennai, presented by Casagrand; YMCA Nandanam, 29 Aug 2026, 4 PM onwards; featured Chinmayi Sripada; poster marks free entry) via `db:seed:event:made-of-chennai-food-music(:live)` → `/chennai-local-events/made-of-chennai-food-music-festival-august-2026`. Poster `public/images/events/made-of-chennai-food-music-festival-august-2026.png`. **Rule:** do not invent a close time when the flyer only says “onwards”; record printed free-entry and partner marks without treating them as a ticket or endorsement.
- **2026-08-23:** AdSense publisher-policy audit — no prohibited adult/hate/weapon/hacking inventory found in repo content; live `ads.txt`, meta `google-adsense-account`, legal routes, and sitemap all pass `npm run adsense:verify`. Tightened `/privacy` + `/cookies` with partner-sites link, Ads Settings / aboutads.info opt-outs, web beacons/IP, no-PII-to-Google, COPPA under-13. **Rule:** AdSense review failures are more often “site not ready / low-value / interstitial / India domain age / identity” than a missing legal page — keep auto-modals off until approved; do not ship syndicated news-agency hero photos without a licence.
- **2026-08-21:** Jobs — weekend turf caretaker (Maduravoyal; Sat 6 PM–1 AM / Sun 2 PM–1 AM; ₹3,000/month as stated) via `db:seed:chennai-job:turf-caretaker-weekend-maduravoyal(:live)` → `/chennai-jobs/turf-caretaker-weekend-maduravoyal`. Source ad had no phone. **Rule:** community weekend listings without a public number use `/contact#jobs` until a phone/WhatsApp is supplied; do not invent the turf name or a higher salary.
- **2026-08-21:** Events — Madras Day 2026 **Chennai-to-Madras** photo exhibition (L Ramachandran, Anna Centenary Library, 22–31 Aug) via `db:seed:event:madras-day-chennai-to-madras(:live)` → `/chennai-local-events/madras-day-chennai-to-madras-photo-exhibition-august-2026`. Poster `public/images/events/madras-day-chennai-to-madras-photo-exhibition-august-2026.png`. **Rule:** use the supplied organiser poster for hub/detail; do not invent gallery hours or a ticket price when the flyer omits them.
- **2026-08-21:** TN Plus Two **Wave A live in repo** — `/guides/tn-plus-two` plus groups 2502/2503/2702/2708/2804/2802, compare, HSE structure, how-to-choose. Sitemap + `llms.txt` P1 entry. Textbooks and Tamil twins still Wave B/E. **Rule:** education pages use `GuideDisclaimer kind="education"`; do not 404-link unbuilt textbook/pathway URLs.
- **2026-08-20:** TN Plus Two education desk **planned** (not built) — 5 pillars + 25 content pages + 2 marketing landings (31 EN URLs) for group selection (DGE 2502 / 2503 / 2702 / 2708 / 2804 / 2802) and official +1/+2 textbook finders. Canonical plan: `.cursor/plans/tn-plus-two-education-desk.plan.md`. **Rule:** booklet is copy inventory only; never rehost SCERT books; EN + `-tamil` twins, not mixed-language pages; M1/M2 must not cannibalise P1/P4.
...
- **2026-08-16:** `/chennai-today` and `/chennai-today-tamil` 500ed in production — Server Components passed `buildText={() => forward}` into client `CopyShareButton` (RSC cannot serialize functions). Pass `text={forward}` instead; `buildText` stays client-only. Also set `dynamic = "force-dynamic"` (live news/events/gold, not 5-min ISR). **Rule:** never pass a function prop from a Server Component to a Client Component; use a string or a client wrapper (`EcrShareButton` pattern).
- **2026-08-16:** News — Chennai local job ads guide for owners/HR via `db:seed:chennai-local-job-ads-guide-business-owners-hr(:live)` → https://mychennaicity.in/chennai-local-news/chennai-local-job-ads-guide-business-owners-hr. Lists Vacancy Chennai, MyChennaiCity Jobs, MyNanganallur (`mynanganallur.in` — not the typo `mynangnallur.in`), MyOMR, MyCovai as **independent local blogs/desks** (not “sister network” copy); Apna + Freshersworld last. Hero Unsplash “We are hiring” (`public/images/articles/chennai-local-job-ads-guide-business-owners-hr-hero.jpg`, [photo](https://unsplash.com/photos/B3UFXwcVbc4)). **Rule:** verify sister-site hostnames with DNS before publishing outbound links; `mynungambakkam.in` / `myannanagar.in` are not live hosts.
- **2026-08-20:** Jobs backlink ops for the hiring guide — MCC inbound via `CHENNAI_JOBS_CAREER_RESOURCES` on `/chennai-jobs`; paste-ready blurbs + placement log for Vacancy Chennai / MyNanganallur / MyOMR / MyCovai + X social draft in [`docs/CHENNAI_JOBS_OUTREACH.md`](CHENNAI_JOBS_OUTREACH.md). **Rule:** one editorial inbound per local desk (not footer farms); mark rows Done in the outreach table when operators place links.
