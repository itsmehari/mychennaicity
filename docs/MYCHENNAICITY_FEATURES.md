...
| **Chennai local events** | `/chennai-local-events`, `/chennai-local-events/[slug]` | Hub lists **`scheduled`** future events from DB when present; otherwise **mock** cards. Detail page resolves a DB slug or `notFound`. Hub uses **events JSON-LD** only when the list is DB-backed. **Rich festival pages:** rows with nullable `presentation_key` + `content_ref` (e.g. `festival_rich`) render an extended layout (schedule, gallery, official PDF strip, FAQ + FAQPage JSON-LD); `NULL` keys keep the legacy prose + standard Event JSON-LD only. Content bundles live under `src/content/special-events/`; resolver in `src/domains/events/presentations/`. |
