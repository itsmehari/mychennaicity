/**
 * Chennai civic news batch — 20 articles (June 2026 editorial).
 *
 * Dev:  `npm run db:seed:chennai-civic-news-batch-2026`
 * Live: `npm run db:seed:chennai-civic-news-batch-2026:live`
 */
...
async function main() {
  ...
  for (const article of CHENNAI_CIVIC_NEWS_BATCH_2026) {
    ...
    if (existing) {
      ...
      updated += 1;
      console.log("[seed-civic-news] Updated:", article.slug);
    } else {
      await db.insert(articles).values({ ...values, createdAt: now });
      inserted += 1;
      console.log("[seed-civic-news] Inserted:", article.slug);
    }
  }

  console.log(
    `[seed-civic-news] Done — inserted ${inserted}, updated ${updated}, total ${CHENNAI_CIVIC_NEWS_BATCH_2026.length}`,
  );

  if (live) {
    await revalidateNewsAfterSeed({ label: "seed-chennai-civic-news-batch-2026" });
  }
}
