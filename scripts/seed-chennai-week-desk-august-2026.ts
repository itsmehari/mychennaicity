/**
 * Chennai week desk — 20 civic articles (8–14 August 2026).
 *
 * Dev:  `npm run db:seed:chennai-week-desk-august-2026`
 * Live: `npm run db:seed:chennai-week-desk-august-2026:live`
 */
...
import { CHENNAI_WEEK_DESK_AUGUST_2026 } from "./content/chennai-week-desk-august-2026";
...
async function main() {
  ...
  for (const article of CHENNAI_WEEK_DESK_AUGUST_2026) {
    ...
    const [existing] = await db
      ...
      .from(articles)
      .where(
        and(eq(articles.cityId, city.id), eq(articles.slug, article.slug)),
      )
      .limit(1);
    ...
    if (existing) {
      ...
      updated += 1;
      console.log("[seed-week-desk] Updated:", article.slug);
    } else {
      await db.insert(articles).values({ ...values, createdAt: now });
      inserted += 1;
      console.log("[seed-week-desk] Inserted:", article.slug);
    }
  }

  console.log(
    `[seed-week-desk] Done — inserted ${inserted}, updated ${updated}, total ${CHENNAI_WEEK_DESK_AUGUST_2026.length}`,
  );
  for (const article of CHENNAI_WEEK_DESK_AUGUST_2026) {
  ...
  if (live) {
    await revalidateNewsAfterSeed({ label: "seed-chennai-week-desk-august-2026" });
  }
}
