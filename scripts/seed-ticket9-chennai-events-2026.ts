/**
 * Bulk seed — Ticket9 Chennai events (Jul–Sep 2026 expanded listing).
 *
 * Dev:  `npm run db:seed:ticket9-chennai-events`
 * Live: `npm run db:seed:ticket9-chennai-events:live`
 */
...
import { TICKET9_CHENNAI_EVENTS_JUL_AUG_2026 } from "../src/content/events/ticket9-chennai-jul-aug-2026";
import {
...
async function main() {
  ...
  for (const e of TICKET9_CHENNAI_EVENTS_JUL_AUG_2026) {
  ...
  console.log(
    `[seed-ticket9-chennai] Done — ${inserted} inserted, ${updated} refreshed (${TICKET9_CHENNAI_EVENTS_JUL_AUG_2026.length} total).`,
  );
  console.log("[seed-ticket9-chennai] Hub: /chennai-local-events");

  await finishListingSeedLive({ label: "ticket9-chennai-events" });
}
