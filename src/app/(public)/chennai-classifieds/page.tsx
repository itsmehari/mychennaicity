...
const hubTitleSegment = "Chennai classified ads";
...
export async function generateMetadata({
  ...
  let total = 0;
  try {
    total = await countOpenClassifiedListingsForChennaiHub();
  } catch {
    total = 0;
  }
  const description =
    total > 0
      ? `${total} reader-submitted classified ads in Chennai — tuition, services, and local wanted posts. Contact advertisers directly.`
      : "Reader-submitted classified ads for Chennai — tuition, services, and neighbourhood wanted posts.";
...
export default async function ChennaiClassifiedsHubPage({
  ...
  try {
    total = await countOpenClassifiedListingsForChennaiHub();
    listings = await listOpenClassifiedListingsForChennaiHub(PAGE_SIZE, offset);
  } catch {
    listings = [];
    total = 0;
  }
    ...
    <div className={interiorMainClassName}>
      ...
      <header className="mt-6 max-w-3xl">
        ...
        <p className="type-lede mt-4 text-base leading-relaxed text-[var(--muted)]">
          Reader-submitted wanted posts and local listings — tuition, services,
          and neighbourhood needs. Contact the advertiser directly; we do not
          broker deals.
        </p>
      </header>
        ...
        <section className="mt-10" aria-label="Open classified ads">
          ...
          {(hasPrev || hasNext) && (
            ...
            >
              {hasPrev ? (
                <Link
                  href={
                    pageNum === 2
                      ? CHENNAI_CLASSIFIEDS_HUB_PATH
                      : `${CHENNAI_CLASSIFIEDS_HUB_PATH}?page=${pageNum - 1}`
                  }
              ...
              {hasNext ? (
                <Link
                  href={`${CHENNAI_CLASSIFIEDS_HUB_PATH}?page=${pageNum + 1}`}
                  className="font-medium text-[var(--accent)] underline-offset-4 hover:underline"
                >
                  Next →
                </Link>
